...
export default function SidebarItem({ item, settingOpen, setSettingOpen }) {
  ...
  const activeItemClasses =
    "text-md m-2 flex items-center justify-start rounded-md p-2 text-gray-500 bg-orange-400 hover:scale-110 hover:cursor-pointer shadow-md shadow-gray-300 text-white";
  return (
    <>
      {item.icon === "setting" && (
        <li
          onClick={toggle}
          className="text-md m-2 flex items-center justify-between gap-x-4 rounded-md p-2 text-gray-500 hover:scale-110 hover:cursor-pointer hover:bg-orange-500 hover:bg-opacity-20 hover:shadow-md"
        >
          <div className="flex">
            <svg
          ...
          {item.children.map((child, index) => {
            return (
              <Link to={child.path} key={index}>
                ...
                >
                  <span className="pl-10">{child.title}</span>
                </li>
              </Link>
            );
          })}
